...
void display(void)   // Creamos la funcion donde se dibuja
{
	...
	//Aqui es donde va la carroceria del carro
	kit.GLrender(NULL, _SHADED, 1.0);  //_WIRED O _POINTS



	glPushMatrix(); //llanta delantera1
		glTranslatef(-6, -1, 7.5);    
		glRotatef(a_llantas, 1, 0, 0);
		llanta.GLrender(NULL, _SHADED, 1.0);
	glPopMatrix();

	glPushMatrix(); //llanta trasera1
		glTranslatef(-6, -1, -9.5); 
		glRotatef(a_llantas, 1, 0, 0);
		llanta.GLrender(NULL, _SHADED, 1.0);
	glPopMatrix();

	glPushMatrix();   //llanta  trasera2
		glTranslatef(6, -1, -9.5);
		glRotatef(180, 0, 1, 0);
		glRotatef(a_llantas, -1, 0, 0);
		llanta.GLrender(NULL, _SHADED, 1.0);
	...
	glPushMatrix();  //llanta delantera2
		glTranslatef(6, -1, 7.5);
		glRotatef(180, 0, 1, 0);
		glRotatef(a_llantas, -1, 0, 0);
		llanta.GLrender(NULL, _SHADED, 1.0);
	...
	glEnable(GL_COLOR_MATERIAL); //glColor funciona con la iluminacion


	glPushMatrix(); //flecha de camino
		glScalef(7, 0.1, 7);
		glDisable(GL_LIGHTING);
...
void animacion()
{
	...
	if (g_fanimacion)
	{
		...
		if (adelante)
		{
			movKit += 1.0;
			rotTires += 10;
			if (movKit >= 130) //este es mi limite
				adelante = false;
