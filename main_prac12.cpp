...
bool circuito = false; //SE ACTIVAN LOS ESTADOS DE MOVIMIENTO
...
void animacion()
{
	...
	if(g_fanimacion)
	{
		if(g_avanza)
		{
			...
			rotTires -= 10;
			if(movKitZ>130)
				g_avanza = false; //SE CAMBIA EL ESTADO DE LA BANDERA (COMENZO EN TRUE)
		}
		else
		{
			movKitZ -=1.0;
	...
	if(circuito)
	{
		...
		{
			rotKit = 220; //NUEVO ANGULO DE ROTACION INCLINADO PARA PASAR DIRECTAMENTE AL PUNTO E
			movKitZ --;
			movKitX--; //A PARTIR DE AQUI SE TRIANGULA LA POSICION
			movKitZ--;//AGREGAR 5 Y 5 DE CADA UNO, INTERPOLARLOS?
			if(movKitZ<-98) //RECORRIDO EN DIAGONAL
			{
				recorrido3 = false;
				recorrido4 = true;
			}
		}
		if(recorrido4) //ESTADO E
		{
			rotKit = 270;
			movKitX --;
			if(movKitX<0)
